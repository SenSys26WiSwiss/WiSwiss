...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'{args.prefix_name}_{args.max_shape[0]}x{args.max_shape[1]}_{args.model_size}_{patch_name}_{shape_name}_{lr_name}_{CURRENT_TIME}')
    ...
    model = InterpViTModel(patch_size=args.patch_size, patch_stride=args.patch_stride, 
                         max_input_shape=args.max_shape, input_channels=1, model_size=args.model_size, 
...
if __name__ == "__main__":
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='interp_study', help='Path to the directory to save checkpoints.')
