...
def train(accelerator, model, train_loader, optimizer, scheduler, epoch, writer, log_interval, logger):
    ...
    total_cnt = torch.tensor(0.0, device=device)
    correct_cnt = torch.tensor(0.0, device=device)
    ...
    for (data, target) in train_loader:
        ...
        if accelerator.sync_gradients:
            cur_bs = data.size(0)
            total_cnt = total_cnt + cur_bs
            correct_cnt = correct_cnt + (output.argmax(dim=1) == target).float().sum()
    ...
    total_cnt = accelerator.gather(total_cnt).sum()
    correct_cnt = accelerator.gather(correct_cnt).sum()
    train_acc = (correct_cnt / total_cnt).item() if total_cnt > 0 else 0.0
    if accelerator.is_main_process:
        logger.info('Train Accuracy: {:.2f}% ({}/{})'.format(100. * train_acc, int(correct_cnt.item()), int(total_cnt.item())))
...
def test(accelerator, model, test_loader, epoch, writer, logger, category_num):
    ...
    correct_cnt = torch.tensor(0.0, device=device)
    total_cnt = torch.tensor(0.0, device=device)
    per_category_correct_cnt = torch.zeros(category_num, device=device)
    per_category_total_cnt = torch.zeros(category_num, device=device)
    with torch.no_grad():
        for data, target in tqdm(test_loader, disable=not accelerator.is_main_process):
            ...
            for i in range(category_num):
                per_category_total_cnt[i] = per_category_total_cnt[i] + (target == i).float().sum()
                per_category_correct_cnt[i] = per_category_correct_cnt[i] + ((output.argmax(dim=1) == target) & (target == i)).float().sum()

    total_cnt = accelerator.gather(total_cnt).sum()
    correct_cnt = accelerator.gather(correct_cnt).sum()
    # per_category: shape (category_num,) per process -> gather gives (num_processes * category_num,) -> reshape and sum
    n_proc = accelerator.num_processes
    per_category_total_cnt = accelerator.gather(per_category_total_cnt).view(n_proc, -1).sum(dim=0)
    per_category_correct_cnt = accelerator.gather(per_category_correct_cnt).view(n_proc, -1).sum(dim=0)
    per_category_acc = (per_category_correct_cnt / (per_category_total_cnt + 1e-8)).cpu().numpy()
    acc = (correct_cnt / total_cnt).item() if total_cnt > 0 else 0.0
    if accelerator.is_main_process:
        logger.info('Per-Category Acc: {}'.format(per_category_acc))
        logger.info('Per-Category correct cnt: {}'.format(per_category_correct_cnt.cpu().numpy()))
        logger.info('Test Accuracy: {:.2f}% ({}/{})'.format(100. * acc, int(correct_cnt.item()), int(total_cnt.item())))
...
def main(args):
    ...
    if accelerator.is_main_process:
        logger.info(f"Total optimizer steps: {num_training_steps}")
