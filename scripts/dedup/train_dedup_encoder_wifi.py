...
def main(args):
    ...
    # set up dataset
    ori_signfi_dataset = HDF5Dataset(args.data_path)
    selected_train_indices = random.sample(range(len(ori_signfi_dataset)), int(len(ori_signfi_dataset) * 0.8))
    selected_signfi_dataset = SubsetDataset(ori_signfi_dataset, selected_train_indices)
    contra_dataset = ContrastiveGaussianDataset(selected_signfi_dataset, [args.gaussian_noise_std])
    contra_loader = ContrastiveDataLoader(contra_dataset, batch_size=args.batch_size, shuffle=True)
    logger.info(f"Train dataset size: {len(selected_signfi_dataset)}")
    logger.info(f"Train dataset indices: {selected_train_indices}")

    # set up model
    model = EncoderCNN(args.embedding_dim, in_channels=1, model_name=args.model_name).to(device)
...
if __name__ == "__main__":
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='embedding_signfi_checkpoints', help='Path to the directory to save checkpoints.')
