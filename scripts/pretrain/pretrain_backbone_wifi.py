...
def main(args):
    ...
    if 'xrf55' in args.pretrain_datasets:
        data_name_list.append(f'XRF55{args.xrf55_input_freq}x{args.xrf55_input_time}')
        ori_length_list.append(1000)
        freq_list.append(200)
    if 'csibench' in args.pretrain_datasets:
        data_name_list.append(f'CSI-Bench{args.csibench_input_freq}x{args.csibench_input_time}')
        ori_length_list.append(1000)
        freq_list.append(30)
    if 'wipose' in args.pretrain_datasets:
        data_name_list.append(f'Wi-Pose{args.wipose_input_freq}x{args.wipose_input_time}')
        ori_length_list.append(15)
        freq_list.append(100)
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'{dfs_name}_{rope_name}_{patch_name}_{args.model_size}_mask{args.mask_ratio}{mask_name}_{data_name}_{bs_name}_lr{args.lr}_{CURRENT_TIME}')
    ...
    for i, dataset_name in enumerate(args.pretrain_datasets):
        ...
        if dataset_name == 'csibench':
            ori_dataset = HDF5Dataset(args.csibench_data_path)
            if args.csibench_subset_indices_path is not None:
                subset_indices = np.load(args.csibench_subset_indices_path)
                train_dataset = SubsetDataset(ori_dataset, subset_indices)
            else:
                train_dataset = ori_dataset
            cur_train_loader = DataLoader(train_dataset, batch_size=args.batch_sizes[i], shuffle=True, num_workers=2)
            pretrain_loader_list.append(cur_train_loader)
            max_batch_cnt = max(max_batch_cnt, len(cur_train_loader))
            logger.info(f"CSI-Bench: train dataset size {len(train_dataset)}")
        
        if dataset_name == 'wipose':
            ori_dataset = HDF5Dataset(args.wipose_data_path)
            if args.wipose_subset_indices_path is not None:
                subset_indices = np.load(args.wipose_subset_indices_path)
                train_dataset = SubsetDataset(ori_dataset, subset_indices)
            else:
                train_dataset = ori_dataset
            cur_train_loader = DataLoader(train_dataset, batch_size=args.batch_sizes[i], shuffle=True, num_workers=2)
            pretrain_loader_list.append(cur_train_loader)
            max_batch_cnt = max(max_batch_cnt, len(cur_train_loader))
            logger.info(f"Wi-Pose: train dataset size {len(train_dataset)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--xrf55_input_freq', type=int, default=96)
    parser.add_argument('--xrf55_input_time', type=int, default=512)

    # CSI-Bench parameters
    parser.add_argument('--csibench_data_path', type=str)
    parser.add_argument('--csibench_subset_indices_path', type=str, default=None)
    parser.add_argument('--csibench_input_freq', type=int, default=96)
    parser.add_argument('--csibench_input_time', type=int, default=512)

    # Wi-Pose parameters
    parser.add_argument('--wipose_data_path', type=str)
    parser.add_argument('--wipose_subset_indices_path', type=str, default=None)
    parser.add_argument('--wipose_input_freq', type=int, default=96)
    parser.add_argument('--wipose_input_time', type=int, default=16)
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='pretrain_mask_checkpoints', help='Path to the directory to save checkpoints.')
