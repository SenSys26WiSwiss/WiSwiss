        ...
        for i in range(len(pretrain_iter_list)):
            ...
            with accelerator.accumulate(model):
                ...
                if cfar_weight > 0 and cfar_instance is not None and cfar_criterion is not None:
                    ...
                    ori_cfar_threshold = cfar_instance(data.squeeze(1))
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        fixed_shape_name = f'{args.fixed_shape[0]}x{args.fixed_shape[1]}x{args.fixed_shape[2]}'
        checkpoint_dir = os.path.join(args.checkpoint_root_dir,
            f'{fixed_shape_name}_{cfar_name}_{rope_name}_{patch_name}_{args.model_size}_mask{args.mask_ratio}{mask_name}_{data_name}_{bs_name}_lr{args.lr}_{CURRENT_TIME}')
    ...
    model = FixedViTModel(patch_size=args.patch_size, patch_stride=args.patch_stride,
                         fixed_input_shape=args.fixed_shape,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fixed_shape', type=lambda s: [int(x) for x in s.split(',')], default=[8,32,32])
    ...
    # training parameters
    parser.add_argument('--batch_sizes', type=lambda s: [int(x) for x in s.split(',')], default=[16,16,16])
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='pretrain_fixedshape_mmwave_checkpoints', help='Path to the directory to save checkpoints.')
