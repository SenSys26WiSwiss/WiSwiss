    ...
    with tqdm(total=eval_steps*len(pretrain_loader_list), desc=f'Train round{epoch}', unit='batch') as pbar:
        while pbar.n < eval_steps*len(pretrain_loader_list):
            for i in range(len(pretrain_iter_list)):
                ...
                epoch_cfar_loss.update(cfar_loss.item(), cur_bs)
                pbar.update(1)
                cur_lr = scheduler.get_last_lr()
                pbar.set_postfix(**{'mse_loss': mse_loss.item(), 'cfar_loss': cfar_loss.item(), 'lr': cur_lr})
                ...
                if pbar.n % log_interval == 0:
                    logger.info('Train Epoch: {} [{}/{} ({:.0f}%)] MSE_loss: {:.6f} CFAR_loss: {:.6f} LR: {}'.format(
                        ...
                        mse_loss.item(), cfar_loss.item(), ', '.join(['{:.6f}'.format(x) for x in cur_lr])))
                    ...
                    writer.add_scalar('Train/cfar_loss', cfar_loss.item(), epoch*eval_steps*len(pretrain_iter_list) + pbar.n)
