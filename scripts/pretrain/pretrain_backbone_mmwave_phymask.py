...
def train(accelerator, model, pretrain_loader_list, optimizer, scheduler, eval_steps,
    ...
    """
    # Sync all ranks before creating iterators (avoids one rank entering next() while another is still saving / not ready).
    ...
    # So that each epoch uses a different shuffle and all ranks stay in sync
    for loader in pretrain_loader_list:
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        phymask_name = f'phymask_{args.phymask_method}_{args.phymask_weights[0]}_{args.phymask_weights[1]}'
        if args.prefix_name:
            phymask_name = args.prefix_name + '_' + phymask_name
        checkpoint_dir = os.path.join(args.checkpoint_root_dir,
            f'{phymask_name}_{cfar_name}_{rope_name}_{patch_name}_{args.model_size}_mask{args.mask_ratio}{mask_name}_{data_name}_{bs_name}_lr{args.lr}_{CURRENT_TIME}')
    ...
    # HDF5 is not safe with multiprocessing (forked workers): can hang when re-iterating in epoch 2+
    ...
    for i, dataset_name in enumerate(args.pretrain_datasets):
        if dataset_name == 'dcdr':
            ...
            cur_train_loader = DataLoader(train_dataset, batch_size=args.batch_sizes[i], shuffle=True)
    ...
    model = RopeViTModelPhyMask(patch_size=args.patch_size, patch_stride=args.patch_stride,
                         input_channels=1, model_size=args.model_size,
                         train_stage=0, pipe_mask=args.pipe_mask,
                         phymask_method=args.phymask_method, phymask_weights=args.phymask_weights,
    ...
    for epoch in range(args.epochs):
        ...
        if accelerator.is_main_process:
            if (epoch + 1) % args.save_interval == 0 or epoch == args.epochs - 1:
                ...
                save_path = os.path.join(checkpoint_dir, f'epoch{epoch}_trainloss{train_loss:.4f}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--phymask_method', type=str, default='weighted', help='Method for PhyMask.')
    parser.add_argument('--phymask_weights', type=tuple, default=(0.5, 0.5), help='Weights for PhyMask.')

    # CFAR parameters
    parser.add_argument('--cfar_weight', type=float, default=0, help='Weight for CFAR loss.')
    ...
    # training parameters
    parser.add_argument('--batch_sizes', type=lambda s: [int(x) for x in s.split(',')], default=[8,8,8])
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='pretrain_phymask_mmwave_checkpoints', help='Path to the directory to save checkpoints.')
