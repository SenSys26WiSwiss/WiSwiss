...
def main(args):
    ...
    if accelerator.is_main_process:
        if not args.resume:
            ...
            shape_name = f'{args.max_shape[0]}x{args.max_shape[1]}x{args.max_shape[2]}'
            checkpoint_dir = os.path.join(args.checkpoint_root_dir,
                f'{shape_name}_{cfar_name}_{patch_name}_{args.model_size}_mask{args.mask_ratio}{mask_name}_{data_name}_{bs_name}_lr{args.lr}_{CURRENT_TIME}')
    ...
    if args.resume:
        ...
        model_resume_path = os.path.join(resume_dir, "latest_model.pth")
        if os.path.isfile(args.resume) and os.path.isfile(model_resume_path):
            optim_scheduler_ckpt = torch.load(args.resume, map_location="cpu")
            unwrapped_model = accelerator.unwrap_model(model)
            unwrapped_model.load_state_dict(torch.load(model_resume_path, map_location="cpu"), strict=True)
            optimizer.load_state_dict(optim_scheduler_ckpt["optimizer_state_dict"])
            scheduler.load_state_dict(optim_scheduler_ckpt["scheduler_state_dict"])
            start_epoch = optim_scheduler_ckpt["epoch"] + 1
            if accelerator.is_main_process:
                logger.info(f"Resumed from {args.resume} and {model_resume_path}, starting at epoch {start_epoch}")
        else:
            if accelerator.is_main_process:
                logger.warning(f"Resume path not found: {args.resume} or {model_resume_path}, starting from epoch 0")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if accelerator.is_main_process:
            unwrapped_model = accelerator.unwrap_model(model)
            # Model checkpoint: state_dict only (same format as before, finetuning code unchanged)
            latest_model_path = os.path.join(checkpoint_dir, "latest_model.pth")
            torch.save(unwrapped_model.state_dict(), latest_model_path)
            # Optimizer and scheduler in a separate file for resume only
            resume_optim_path = os.path.join(checkpoint_dir, "resume_optim_scheduler.pt")
            torch.save({
            ...
            if (epoch + 1) % args.save_interval == 0 or epoch == args.epochs - 1:
                ...
                logger.info(f"Saved model checkpoint and resume state to {checkpoint_dir}")
...
if __name__ == '__main__':
    ...
    # training parameters
    parser.add_argument('--batch_sizes', type=list, default=[4,4,4])
    ...
    # log parameters
    parser.add_argument('--checkpoint_root_dir', type=str, default='pretrain_interp_mmwave_checkpoints', help='Path to the directory to save checkpoints.')
    ...
    parser.add_argument('--resume', type=str, default=None, help='Path to resume state file (resume_optim_scheduler.pt) to continue training. Model is loaded from latest_model.pth in the same directory.')
