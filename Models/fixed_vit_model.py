"""
ViT with fixed input size (pos_embed only, no RoPE).
Same 3-stage flow as RopeViTModel: pre-training, fine-tuning, train from scratch.
...
class FixedViTModel(nn.Module):
    """
    ViT with learned absolute position embedding (APE) and fixed input spatial size.
    No RoPE; uses pos_embed only like APEViT2DModel.
    - fixed_input_shape: (H, W) for 2D or (D, H, W) for 3D.
    - Optional resize to fixed shape when input size differs (resize_to_fixed=True).
    """
    def __init__(self, patch_size, patch_stride, fixed_input_shape, 
                 input_channels, model_size, 
                 train_stage, task_type=0, num_anchors=-1, 
        ...
        self.resize_to_fixed = resize_to_fixed
        self._ndim = len(self.fixed_input_shape)
        if self._ndim not in (2, 3):
            raise ValueError("fixed_input_shape must be 2D (H,W) or 3D (D,H,W).")
        ...
        if train_stage == 0:
            ...
            pretrained_model = FixedViTModel(patch_size=patch_size, patch_stride=patch_stride, 
            ...
            if self.task_type == 0:
                ...
                # get mid shape
                grid_shape = self._get_mid_shape(self.patch_embed.proj, [self.input_channels] + fixed_input_shape)
        ...
        self._fixed_grid_shape = self._get_mid_shape(self.patch_embed.proj, [self.input_channels] + fixed_input_shape)
    ...
    def _init_components(self):
        ...
        mid = self._get_mid_shape(self.patch_embed.proj, [self.input_channels] + self.fixed_input_shape)
        num_patches = int(np.prod(mid))
    ...
    def ensure_fixed_shape(self, x):
        if self._ndim == 2:
            cur_h, cur_w = x.shape[2], x.shape[3]
            tgt_h, tgt_w = self.fixed_input_shape[0], self.fixed_input_shape[1]
            if (cur_h, cur_w) != (tgt_h, tgt_w):
                if not self.resize_to_fixed:
                    raise ValueError(
                        f"Expected spatial shape {self.fixed_input_shape}, got ({cur_h}, {cur_w})."
                    )
                x = F.interpolate(x, size=(tgt_h, tgt_w), mode="bilinear", align_corners=False)
        else:
            cur_d, cur_h, cur_w = x.shape[2], x.shape[3], x.shape[4]
            tgt_d, tgt_h, tgt_w = self.fixed_input_shape[0], self.fixed_input_shape[1], self.fixed_input_shape[2]
            if (cur_d, cur_h, cur_w) != (tgt_d, tgt_h, tgt_w):
                if not self.resize_to_fixed:
                    raise ValueError(
                        f"Expected spatial shape {self.fixed_input_shape}, got ({cur_d}, {cur_h}, {cur_w})."
                    )
                x = F.interpolate(x, size=(tgt_d, tgt_h, tgt_w), mode="trilinear", align_corners=False)
        return x
    ...
    def yolo_detect(self, x):
        ...
        backbone_feature = backbone_feature.reshape(B, self.embed_dim, *self._fixed_grid_shape)
        backbone_feature = backbone_feature.transpose(1, 2)
        backbone_feature = backbone_feature.reshape(B, self._fixed_grid_shape[0]*self.embed_dim, *self._fixed_grid_shape[1:])
    ...
    def pretrain_mask(self, x, cluster, mask_patch_ratio):
        ...
        grid_shape = self._fixed_grid_shape
